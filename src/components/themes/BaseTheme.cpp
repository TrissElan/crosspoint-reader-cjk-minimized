...
void BaseTheme::drawBattery(const GfxRenderer& renderer, Rect rect, const bool showPercentage,
  ...
  if (showPercentage) {
    ...
    if (iconOnLeft) {
      renderer.drawText(UI_12_FONT_ID, rect.x + batteryPercentSpacing + BaseMetrics::values.batteryWidth, rect.y,
  ...
  drawBatteryIcon(renderer, rect.x, iconY, BaseMetrics::values.batteryWidth, rect.height, percentage);
}
...
void BaseTheme::drawProgressBar(const GfxRenderer& renderer, Rect rect, const size_t current,
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, rect.y + rect.height + 15, percentText.c_str());
}
...
void BaseTheme::drawSideButtonHints(const GfxRenderer& renderer, const char* topBtn, const char* bottomBtn) const {
  ...
  if (gpio.deviceIsX3()) {
    ...
    if (topBtn != nullptr && topBtn[0] != '\0') {
      ...
      renderer.drawTextRotated90CW(UI_12_FONT_ID, textX, textY, topBtn);
    }
    ...
    if (bottomBtn != nullptr && bottomBtn[0] != '\0') {
      ...
      const int textY = x3ButtonY + (buttonHeight + textWidth) / 2;
      renderer.drawTextRotated90CW(UI_12_FONT_ID, textX, textY, bottomBtn);
    }
  } else {
    ...
    for (int i = 0; i < 2; i++) {
      if (labels[i] != nullptr && labels[i][0] != '\0') {
        ...
        renderer.drawTextRotated90CW(UI_12_FONT_ID, textX, textY, labels[i]);
      }
    }
  }
}
...
void BaseTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    auto font = (rowSubtitle != nullptr) ? UI_12_FONT_ID : UI_12_FONT_ID;
    ...
    if (rowSubtitle != nullptr) {
      ...
      auto subtitle = renderer.truncatedText(UI_12_FONT_ID, subtitleText.c_str(), textWidth);
      renderer.drawText(UI_12_FONT_ID, rect.x + BaseMetrics::values.contentSidePadding, itemY + 30, subtitle.c_str(),
                        i != selectedIndex);
    }
...
void BaseTheme::drawRecentBooks(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
                                const int selectorIndex) const {
  if (recentBooks.empty()) {
    const int y = rect.y + (rect.height - renderer.getLineHeight(UI_12_FONT_ID) * 2) / 2;
    renderer.drawCenteredText(UI_12_FONT_ID, y, "No open book");
    renderer.drawCenteredText(UI_12_FONT_ID, y + renderer.getLineHeight(UI_12_FONT_ID), "Start reading below");
    return;
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (selected) {
      renderer.fillRect(rect.x, entryY, rect.width, entryHeight, true);
    }
    const int textX = rect.x + BaseMetrics::values.contentSidePadding;
    const int textWidth = rect.width - 2 * BaseMetrics::values.contentSidePadding;
...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  if (SETTINGS.statusBarBookProgressPercentage || SETTINGS.statusBarChapterPageCount) {
    ...
    progressTextWidth = renderer.getTextWidth(UI_12_FONT_ID, progressStr);
    renderer.drawText(
        UI_12_FONT_ID,
  ...
  if (!title.empty()) {
    ...
    int titleWidth;
    titleWidth = renderer.getTextWidth(UI_12_FONT_ID, title.c_str());
    ...
    if (titleWidth > availableTitleSpace) {
      title = renderer.truncatedText(UI_12_FONT_ID, title.c_str(), availableTitleSpace);
      titleWidth = renderer.getTextWidth(UI_12_FONT_ID, title.c_str());
    }
...
void BaseTheme::drawKeyboardKey(const GfxRenderer& renderer, Rect rect, const char* label,
  ...
  if (isSelected) {
    renderer.drawText(UI_12_FONT_ID, textX - 6, rect.y, "[");
    renderer.drawText(UI_12_FONT_ID, textX + itemWidth, rect.y, "]");
  }
  renderer.drawText(UI_12_FONT_ID, textX, rect.y, label);
}
