...
void EpubReaderMenuActivity::render(RenderLock&&) {
  ...
  auto drawContent = [&]() {
    ...
    const std::string truncTitle =
        renderer.truncatedText(UI_12_FONT_ID, title.c_str(), contentWidth - 40);
    // Manual centering so we can respect the content gutter.
    const int titleX =
        contentX + (contentWidth - renderer.getTextWidth(UI_12_FONT_ID, truncTitle.c_str())) / 2;
    renderer.drawText(UI_12_FONT_ID, titleX, 15 + contentY, truncTitle.c_str(), true);
